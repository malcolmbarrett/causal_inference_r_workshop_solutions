...
```{r}
propensity_model <- glm(net ~ income + health + temperature, data = net_data, family = binomial())
...
ggplot(df, aes(x = health, color = as.factor(net))) +
  geom_ecdf(aes(weights = w_atm))

ggplot(df, aes(x = temperature, color = as.factor(net))) +
  geom_ecdf(aes(weights = w_atm))
...
```{r}
standardized_model <- lm(malaria_risk ~ net + income + health + temperature, data = net_data)

yes <- mutate(net_data, net = TRUE)
no <- mutate(net_data, net = FALSE)
